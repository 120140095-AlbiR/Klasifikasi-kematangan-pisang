...
def gray_world(img_bgr):
    """Gray World White Balance untuk koreksi kecerahan (Sama persis dengan original)"""
...
def show_img_streamlit(img, title=None):
    """Helper untuk menampilkan gambar seperti show_img original tapi di Streamlit"""
    if title:
        st.caption(f"**{title}**")
    ...
    if len(img.shape) == 3:
        ...
        st.image(img_rgb, clamp=True, use_container_width=True)
    else:
        # Grayscale
        st.image(img, clamp=True, use_container_width=True, channels='GRAY')
...
if uploaded_file is not None:
    ...
    # Menampilkan channel satu per satu seperti script asli
    c1, c2 = st.columns(2)
...
Mean b* (area tersegmentasi): {mean_bstar:.2f} (std: {std_bstar:.2f})
    ...
    if mask_bool.any():
        ...
        plt.tight_layout()
        st.pyplot(fig) # Render plot Matplotlib ke Streamlit
    ...
    # Logika Klasifikasi (Sama persis)
    if 74 <= hue_standar <= 120 and 28 <= bstar_standar <= 69.9:
        maturity_status = 'Mentah (R2-R3)'
        color_desc = 'Hijau tua (sesuai input Anda)'

    elif 64 <= hue_standar <= 73.9 and 70 <= bstar_standar <= 84.9:
        maturity_status = 'Setengah Matang (R4-R5)'
        color_desc = 'Kuning kehijauan (sesuai input Anda)'

    elif 30 <= hue_standar <= 63.9 and 85 <= bstar_standar <= 120:
        maturity_status = 'Matang Penuh (R6-R7)'
        color_desc = 'Kuning cerah (sesuai input Anda)'

    else:
        diff_mentah = abs(hue_standar - 110) + abs(bstar_standar - 32)
        diff_setengah = abs(hue_standar - 82) + abs(bstar_standar - 38)
        diff_matang = abs(hue_standar - 65) + abs(bstar_standar - 78)
        
        min_diff = min(diff_mentah, diff_setengah, diff_matang)
        if min_diff == diff_mentah:
            maturity_status = 'Mentah (R2-R3)'
            color_desc = 'Hijau tua (terdekat)'
        elif min_diff == diff_setengah:
            maturity_status = 'Setengah Matang (R4-R5)'
            color_desc = 'Kuning kehijauan (terdekat)'
        else:
            maturity_status = 'Matang Penuh (R6-R7)'
            color_desc = 'Kuning cerah (terdekat)'

    st.markdown("---")
    st.success(f"### STATUS KEMATANGAN: {maturity_status}")
    st.info(f"**Warna Dominan:** {color_desc}")
    st.markdown("---")

    # ==================== VISUALISASI AKHIR ====================
    st.markdown("### === Visualisasi Hasil Akhir + Contoh Warna Pisang ===")
